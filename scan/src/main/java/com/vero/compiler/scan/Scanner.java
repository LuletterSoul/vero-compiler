...
import com.vero.compiler.scan.exception.ScanException;
import com.vero.compiler.scan.lexer.Lexeme;
import com.vero.compiler.scan.source.SourceLocation;
...
public class Scanner
{
    ...
    public Scanner(ScannerInfo scannerInfo)
    {
        ...
        this.engine = new FiniteAutomationMachineEngine(scannerInfo.getCharClassTable(),
            scannerInfo.getTransitionTable());
        this.lexemeValueBuilder = new StringBuilder();
        this.tokenAttributes = new Integer[scannerInfo.getTokenNum()];
    ...
    public Scanner(ScannerInfo scannerInfo, SourceReader sourceReader)
    {
        this(scannerInfo);
    ...
    public Lexeme read()
    {
        ...
        while (true)
        {
            ...
            if (this.sourceReader.peekChar() == Character.MAX_VALUE)
            {
                ...
                addHistory(new Lexeme(this.scannerInfo, this.scannerInfo.getEndOfStreamState(),
            ...
            {
                addHistory(new Lexeme(this.scannerInfo, this.lastState,
    ...
    {
        int acceptTokenIndex = getScannerInfo().getTokenIndex(getLastState());
        //
