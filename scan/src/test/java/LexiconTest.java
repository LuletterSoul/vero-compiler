...
//        Token XMLNS = xml.defineToken(Literal("xmlns"));
        ...
        Integer[][] transitionTable = tc.getRealCompressedTransitionTable();
        for (Integer[] aTransitionTable : transitionTable)
        {
            for (Integer anATransitionTable : aTransitionTable)
            {
                System.out.print(anATransitionTable.toString().concat(","));
            }
            System.out.println();
        }
//        FiniteAutomationMachineEngine machineEngine = new FiniteAutomationMachineEngine();
    }
}
